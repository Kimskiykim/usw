...
Для structured root run references SHALL содержать exact run ID и
project-relative путь его machine checkpoint. Nested boundaries MUST NOT
создавать отдельные HANDOFF operations.
...
#### Scenario: Structured operation
- **WHEN** HANDOFF начинает или продолжает structured root run
- **THEN** его references однозначно указывают на `.usw/states/flows/<origin>/<flow-name>/<run-id>/flow.json`
...
#### Scenario: Run-scoped checkpoint текущей операции
- **WHEN** run-scoped checkpoint совпадает с exact run ID и reference активного HANDOFF
- **THEN** его наличие не блокирует Begin или resume той же root operation

#### Scenario: Другой run-scoped checkpoint
- **WHEN** active HANDOFF ссылается на другой run ID
- **THEN** orchestrator не выбирает checkpoint по имени flow и не продолжает чужой run автоматически
