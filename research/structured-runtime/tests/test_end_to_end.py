...
"""
            ...
            shared_path = RUNNER.save_custom_checkpoint(
                project,
                shared,
            ...
            with self.assertRaisesRegex(RUNNER.CustomFlowError, "stale_flow"):
                RUNNER.resume_custom_state(
                    local,
                    RUNNER.load_custom_checkpoint(
                        project, shared, shared_path.parent.name
                    ),
                    current_source_identity=None,
                )
            local_path = RUNNER.save_custom_checkpoint(
                project,
                local,
                RUNNER.FlowState(0, "task/x", False),
                source_identity=None,
            )
            self.assertNotEqual(shared_path, local_path)
            self.assertIn("/shared/", shared_path.as_posix())
            self.assertIn("/local/", local_path.as_posix())
