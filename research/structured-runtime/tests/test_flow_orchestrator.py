...
""",
    ...
    def test_structured_checkpoint_preserves_binding(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            checkpoint = RUNNER.load_custom_checkpoint(
                project, flow, path.parent.name
            )
    ...
    def test_custom_checkpoint_round_trip_and_fresh_resume(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                project
                / ".usw/states/flows/shared"
                / flow.name
                / path.parent.name
                / "flow.json",
                path,
            )
            self.assertEqual(path.parent.name, checkpoint.run_id)
            self.assertEqual("shared", checkpoint.flow_origin)
    ...
    def test_custom_runs_have_distinct_exact_checkpoints(self):
        flow = self.custom(
            "1. Скилл: `first-skill`\n"
            "   - Пишет: нет"
        )
        with tempfile.TemporaryDirectory() as directory:
            project = Path(directory)
            (project / ".usw").mkdir()
            first = RUNNER.save_custom_checkpoint(
                project,
                flow,
                RUNNER.FlowState(0, "task/x"),
                source_identity=None,
            )
            second = RUNNER.save_custom_checkpoint(
                project,
                flow,
                RUNNER.FlowState(0, "task/y"),
                source_identity=None,
            )
            ...
            self.assertEqual(
                "task/x",
                RUNNER.load_custom_checkpoint(
                    project, flow, first.parent.name
                ).selected_scope,
            )
            self.assertEqual(
                "task/y",
                RUNNER.load_custom_checkpoint(
                    project, flow, second.parent.name
                ).selected_scope,
            )

    def test_legacy_singleton_checkpoint_remains_readable(self):
        flow = self.custom(
            "1. Скилл: `first-skill`\n"
            "   - Пишет: нет"
        )
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual("task/legacy", checkpoint.selected_scope)
            self.assertIsNone(checkpoint.run_id)

    def test_run_checkpoint_rejects_unsafe_selector_and_state_directory(self):
        flow = self.custom(
            "1. Скилл: `first-skill`\n"
            "   - Пишет: нет"
        )
        with tempfile.TemporaryDirectory() as directory:
            project = Path(directory)
            (project / ".usw").mkdir()
            with self.assertRaisesRegex(RUNNER.CustomFlowError, "invalid_run_id"):
                RUNNER.load_custom_checkpoint(project, flow, "../unsafe")

            victim = project / "victim"
            victim.mkdir()
            os.symlink(victim, project / ".usw/states")
            with self.assertRaisesRegex(RUNNER.CustomFlowError, "invalid_local_state"):
