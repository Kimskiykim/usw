...
def _safe_run_id(value: str) -> str:
    try:
        ...
        raise CustomFlowError("invalid_run_id", f"invalid run UUID: {value!r}") from error
    if str(parsed) != value:
        raise CustomFlowError("invalid_run_id", f"run UUID must be canonical: {value!r}")
    return value


def _safe_state_directory(path: Path, *, create: bool) -> None:
    try:
        mode = os.lstat(path).st_mode
    except FileNotFoundError:
        if not create:
            raise CustomFlowError("missing_checkpoint", f"flow state directory is missing: {path}")
        try:
            os.mkdir(path, 0o700)
    ...
    if not stat.S_ISDIR(mode) or stat.S_ISLNK(mode):
        raise CustomFlowError("invalid_local_state", f"unsafe flow state directory: {path}")


def _custom_checkpoint_path(
    project_root: Path,
    flow: CustomFlow | None = None,
    run_id: str | None = None,
    *,
    create: bool = False,
) -> Path:
    ...
    try:
        _safe_state_directory(local_root, create=False)
    except CustomFlowError as error:
        if error.code == "missing_checkpoint":
            raise CustomFlowError(
                "missing_local_state", f"local state is missing: {local_root}"
            ) from error
        raise
    if flow is None and run_id is None:
        path = local_root / "FLOW.json"
    elif flow is None or run_id is None:
        raise CustomFlowError(
            "missing_run_selector", "flow and run UUID must be provided together"
        )
    else:
        if flow.origin not in CUSTOM_FLOW_ORIGINS or not CUSTOM_FLOW_NAME.fullmatch(flow.name):
            raise CustomFlowError("invalid_checkpoint", "flow has an unsafe state identity")
        current = local_root
        for part in ("states", "flows", flow.origin, flow.name, _safe_run_id(run_id)):
            current /= part
            _safe_state_directory(current, create=create)
        path = current / "flow.json"
    try:
...
def save_custom_checkpoint(
    ...
    *,
    ...
    run_id = _safe_run_id(run_id) if run_id is not None else str(uuid.uuid4())
    path = _custom_checkpoint_path(project_root, flow, run_id, create=True)
    if path.exists():
        existing = load_custom_checkpoint(project_root, flow, run_id)
        if (
            existing.flow_identity != flow.identity
            or existing.selected_scope != state.selected_scope
        ):
            raise CustomFlowError(
                "run_conflict", "run UUID already belongs to different flow state"
            )
    schema_version = (
        3
        if flow.version == STRUCTURED_FLOW_VERSION
...
def load_custom_checkpoint(
    ...
    flow: CustomFlow | None = None,
    run_id: str | None = None,
) -> CustomFlowCheckpoint:
    path = _custom_checkpoint_path(project_root, flow, run_id)
    try:
    ...
    run_fields = {"flow_origin", "run_id"}
    present_run_fields = run_fields & set(data) if isinstance(data, dict) else set()
    if present_run_fields and present_run_fields != run_fields:
        raise CustomFlowError("invalid_checkpoint", "incomplete run identity")
    if present_run_fields:
        expected.update(run_fields)
    ...
    if (
        ...
        or (
            present_run_fields
            and (
                data["flow_origin"] not in CUSTOM_FLOW_ORIGINS
        ...
        raise CustomFlowError("invalid_checkpoint", "invalid checkpoint values")
    if present_run_fields:
        _safe_run_id(data["run_id"])
    if run_id is None and present_run_fields:
        raise CustomFlowError("invalid_checkpoint", "run-scoped state is stored at the legacy path")
    if run_id is not None and (
        not present_run_fields
        or data["run_id"] != run_id
        or data["flow_origin"] != flow.origin
    ):
        raise CustomFlowError("invalid_checkpoint", "checkpoint path and run identity differ")
    loop_counts: tuple[tuple[str, int], ...] = ()
    if data["schema_version"] in {2, 3}:
...
def resume_custom_state(
    ...
    *,
    ...
    if checkpoint.flow_origin is not None and checkpoint.flow_origin != flow.origin:
        raise CustomFlowError("stale_flow", "flow origin changed after the checkpoint was saved")
    expected_schemas = (
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "checkpoint-save":
            ...
            _print_json({"checkpoint": str(path), "run_id": path.parent.name})
            return 0
        checkpoint = load_custom_checkpoint(
            args.project_root,
            flow if args.run_id is not None else None,
            args.run_id,
        )
