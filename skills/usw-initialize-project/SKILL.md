...
3. Использовать первого кандидата, завершившегося успешно, и передать ему
   текущую открытую папку как `<project>` единственным аргументом.
...
README каталога примеров, `.usw/.gitignore` и, при включённом handoff,
`.usw/HANDOFF.md` созданы или уже существовали. Никогда не перезаписывать
существующий файл. `.usw/flows/` и `.usw/refinements/` создаются только при
первом локальном flow или уточнении намерения.
...
В тот же каталог скопировать `README.md`, поясняющий назначение примеров.
