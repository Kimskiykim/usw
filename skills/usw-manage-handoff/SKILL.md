...
Использовать переданный корень открытого проекта буквально, не искать `.git` и
не подниматься к родительскому каталогу. Прочитать поле верхнего уровня
`handoff` из `<project-root>/usw.yaml`. Отсутствующее поле означает `true`.
