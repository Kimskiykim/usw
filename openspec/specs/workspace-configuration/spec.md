...
`artifacts.root` MAY содержать специализированные refinement, flow и review
roots только при provider `standalone`. Специализированные roots MUST быть
попарно непересекающимися. Любое другое равенство или ancestor/descendant
overlap между managed roots, а также совпадение или overlap с project root,
...
#### Scenario: Standalone namespace содержит специализированные roots
- **WHEN** standalone configuration использует `artifacts.root: usw` и default
  refinement, flow и review roots под `usw/`
- **THEN** ожидаемое containment принимается, потому что специализированные roots
  различны и не пересекаются друг с другом

#### Scenario: Специализированные roots пересекаются
- **WHEN** review root совпадает с flow root либо один специализированный root
  содержит другой
...
- **WHEN** configuration явно выбирает provider `openspec` и не задаёт
  `artifacts.root`
- **THEN** planning root разрешается как `openspec`, а refinement, flow и review
  roots остаются `usw/refinements`, `usw/flows` и `usw/reviews`
