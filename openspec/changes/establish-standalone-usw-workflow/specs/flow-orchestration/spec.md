...
- **WHEN** `proposal` и capability `specs` завершены, `design` готов, а `tasks`
  заблокирован нативным graph
...
- **THEN** Development создаёт technical `design`, после чего создаёт ставший
  доступным `tasks.md`

### Requirement: Development сохраняет authority спецификации
Development SHALL владеть technical design, executable planning, implementation,
...
- **WHEN** task definition of done, plan и обязательные local checks выполнены
...
- **THEN** затронутое evidence становится stale, Testing повторяется, а затем
...
small-step planning и handoff skills и добавлять только provider-aware artifact
