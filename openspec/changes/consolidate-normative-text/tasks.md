...
- [x] 1.3 For each invariant that a scenario can express, add the scenario and observe it passing. Remaining assertions stay untouched in this task (the create-flow assertions were already removed in 3.5; this task covers the other skills). Historical ordering: the run-flow scenarios existed before 3.1, while finder/assessor coverage landed after their 3.4 restructure; manage-handoff remains scenario-uncovered for the durable-state limitation documented in 1.4. Done 2026-08-21: `find-does-not-execute` observes that discovery returns an explicit-origin run command without creating the fixture flow's sentinel; `assess-does-not-read-siblings` treats a named package resource as `unverified` without echoing its sibling-only canary. Same isolated `{workdir}` Codex runner, three runs each: both scenarios `3/3 [pass]`. The assessor canary is behavioral evidence rather than syscall proof and does not verify use of the `inspect` loader boundary; those limits are stated in the scenario notes.
...
- [x] 4.2 Remove stray English sentences from the normative bodies of the other skills and the specs, leaving technical tokens in place. Completed 2026-08-21 after the author's explicit choice to translate the previously recorded residual: all fourteen main specs and both delta specs of this change now use Russian normative prose. OpenSpec parser markers (`Requirements`, `Requirement`, `Scenario`, `WHEN`, `THEN`, `ADDED`, `MODIFIED`) and established technical tokens remain unchanged. Command stubs under `commands/` are English dispatch metadata outside the normative-body scope.
- [x] 4.3 Confirm no shipped skill mixes languages in its normative body and that frontmatter `description` fields are untouched. Confirmed: all six skills, their references, all fourteen main specs and both delta specs use Russian normative prose with technical tokens in original form; every skill frontmatter `description` remains byte-identical to its pre-change value.
...
- [x] 5.2 Run the complete suite on the supported Python floor and latest, `openspec validate --all --strict`, `openspec status --change consolidate-normative-text --json` and `git diff --check`, recording each result. Repeated after the full spec translation on 2026-08-21: Python 3.10.19 (supported floor) `250/250 OK`; Python 3.13.12 (CI latest) `250/250 OK`; strict OpenSpec validation `18 passed, 0 failed`; status JSON reported `schemaName: spec-driven`, `isComplete: true` and all four planning artifacts `done`; `git diff --check` exited 0 with no output.
